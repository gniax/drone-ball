...
public class ShipCollision : Resettable
{
    ...
    private void DoCarCarInteraction(Collision collisionInfo)
    {
        ...
        Vector3 directionToOtherCogLow = collisionInfo.transform.Find("ShipController").Find("cogLow").position -
                                         transform.Find("ShipController").Find("cogLow").position;
    ...
    void RestoreBumperPhysics(GameObject bumpedCar, Vector3 directionToOtherCogLow, Collision col)
    {
        ...
        bumpedCar.GetComponent<ShipCollision>().CancelUnityImpulse();
