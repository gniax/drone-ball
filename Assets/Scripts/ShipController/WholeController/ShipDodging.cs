...
public class ShipDodging : MonoBehaviour
{
    const float DodgeDeadzone = 0.50f;

    private bool _isFirstDodgePress = false;
    ...
    public bool disableDoubleDodge;
    ...
    {
        UpdateDodgeVariables();
        Dodge();
        RotateBackToTheFeet();
    }

    private void UpdateDodgeVariables()
    {
        bool carIsGrounded = _controller.shipState == ShipController.ShipStates.AllCornersSurface 
                          || _controller.shipState == ShipController.ShipStates.Ground;
        ...
        //if (!carIsGrounded && _inputManager.isJump && !_isSecondJumpUsed && _timerSecondJump > 0f && _lastFrameNoButton && !disableDoubleDodge)
        //{
        //    isSecondJump = true;
    ...
    {
        // add initial forces
        if (_isFirstDodgePress)
        {
            _isFirstDodgePress = false;
            _rb.AddForce(_rb.transform.up * 2.99f, ForceMode.VelocityChange);
        }
        // add acceleration if the dodge button is held down
        else
        {
